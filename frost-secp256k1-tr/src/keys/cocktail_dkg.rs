...
pub mod round1 {
    ...
    pub type SecretPackage = frost::keys::cocktail_dkg::round1::SecretPackage<S>;

    /// The package that must be broadcast by each participant to all other participants
    /// (via the coordinator) between Round 1 and Round 2.
    pub type Package = frost::keys::cocktail_dkg::round1::Package<S>;
}
...
pub mod round2 {
    ...
    ///
    /// This package MUST NOT be sent to other participants!
    pub type SecretPackage = frost::keys::cocktail_dkg::round2::SecretPackage<S>;

    /// The package sent by each participant to the coordinator after Round 2,
    /// containing the transcript signature.
    pub type Package = frost::keys::cocktail_dkg::round2::Package<S>;
}
