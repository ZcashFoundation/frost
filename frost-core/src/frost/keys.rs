...
use crate::{
    frost::Identifier, random_nonzero, Ciphersuite, Element, Error, Field, Group, GroupError,
    Scalar, VerifyingKey,
};
...
// Add a deserialize() to VerifiableSecretSharingCommitment
// that takes a vector of encoded CoefficientCommitment/Element .
// Then add a serialize() that returns the same vector. This allows hiding CoefficientCommitment from the caller, it won't need to be pub
