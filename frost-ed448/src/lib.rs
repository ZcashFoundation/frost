...
pub mod keys {
    ...
    ///
    /// This is identical to [`keygen_with_dealer`] but receives an existing key
    ...
    /// seed phrase.
    pub fn split_key<R: RngCore + CryptoRng>(
        secret: &SigningKey,
        max_signers: u16,
        min_signers: u16,
        rng: &mut R,
    ) -> Result<(HashMap<Identifier, SecretShare>, PublicKeyPackage), Error> {
        frost::keys::split_key(secret, max_signers, min_signers, rng)
    }
    ...
    pub fn reconstruct_key(secret_shares: Vec<SecretShare>) -> Result<SigningKey, Error> {
        frost::keys::reconstruct_key(secret_shares)
    }
